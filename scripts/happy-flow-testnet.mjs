...
async function exec(client, signer, makeTx, label) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(
        `[retry] ${label} attempt ${attempt}/${maxAttempts} failed with transient RPC error. Retrying in ${delayMs}ms...`
      );
      await sleep(delayMs);
    }
  }
...
async function main() {
  ...
  const gasReserveMist = BigInt(process.env.HAPPY_FLOW_GAS_RESERVE_MIST || "150000000");
  const budgetMist = BigInt(process.env.HAPPY_FLOW_BUDGET_MIST || "200000000");
  const rewardMist = BigInt(process.env.HAPPY_FLOW_REWARD_MIST || "100000000");
  const walrusEpochs = Number(process.env.HAPPY_FLOW_WALRUS_EPOCHS || "3");
  const walrusRelayHost =
    process.env.HAPPY_FLOW_WALRUS_RELAY_HOST || "https://upload-relay.testnet.walrus.space";
  ...
  console.log("Running AgentGuild happy flow on testnet");
  console.log(`sender=${sender}`);
  console.log(`packageId=${packageId}`);
  console.log(`boardId=${boardId}`);
  console.log(`budgetMist=${budgetMist.toString()} rewardMist=${rewardMist.toString()}`);

  const unique = Date.now();
  const handle = `agent-${unique}`;
  const title = `Happy Flow ${unique}`;

  if (rewardMist > budgetMist) {
    throw new Error("HAPPY_FLOW_REWARD_MIST must be <= HAPPY_FLOW_BUDGET_MIST");
  }
    ...
    () => {
      ...
      tx.moveCall({
        ...
        arguments: [
          ...
          tx.pure.string(`https://example.com/${unique}.json`),
        ],
      });
      return tx;
    },
    ...
    () => {
      ...
      tx.moveCall({
        ...
        arguments: [
          ...
          tx.pure.string("Automated happy flow task"),
  ...
  const fundRes = await exec(
    ...
    async () => {
      ...
      if (maxSpendable <= 0n) {
        throw new Error(
          `Insufficient SUI in largest coin for fund + gas reserve. largest=${coin.balance.toString()} reserve=${gasReserveMist.toString()}`
        );
      }
      const fundMist = budgetMist <= maxSpendable ? budgetMist : maxSpendable;
      if (fundMist < budgetMist) {
        throw new Error(
      ...
      const [payment] = tx.splitCoins(tx.gas, [tx.pure.u64(fundMist)]);
      tx.moveCall({
  ...
  const submitArtifactRes = await exec(
    client,
    signer,
    async () => {
      const artifactPayload = `happy-flow:${unique}:task=${taskId}:role=${roleId}:sender=${sender}`;
  ...
  const artifactId = await pickCreatedObjectId(
    client,
    submitArtifactRes,
    "::artifact::Artifact"
  );
  ...
  console.log("Happy flow completed.");
...
main().catch((err) => {
  console.error("Happy flow failed:");
  console.error(err);
  process.exit(1);
});
